"""FEED REAL — mesma bancada, sem a assistant-ui, com os RENDERERS DE VERDADE.

Terceiro braço, derivado de escala_g1_sem_lib.py: mesmo método, mesma janela,
mesma rotação, mesma carga. A única diferença é a rota — /spike/feed monta
`components/feed/**`, com markdown, execução e raciocínio de verdade, no lugar
do DOM feio que o braço de controle usava para isolar a biblioteca.

A pergunta AQUI é outra: o feed real segura o p95 que o esqueleto segurava,
agora que cada item custa o que vai custar em produção? O G1 já respondeu que a
biblioteca era o gargalo (33,4 ms contra 400–724,9; escala 1,81× → 1,00×). O que
falta saber é quanto do orçamento os renderers consomem — e piorar aqui é um
resultado legítimo, desde que medido e dito.
...
BASE = 'http://127.0.0.1:3008/spike/feed'
