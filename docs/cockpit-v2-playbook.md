...
- ~~**onde constrói:** Oracle (`vps-arm-borges-767247`)~~ — **CORRIGIDO em 30/07, ver `cockpit-v2-ownership.md` §5.** A Oracle continua com o dobro de memória e o triplo de disco, mas **não alcança o back**: o `uvicorn` escuta somente `127.0.0.1:8000`, e da Oracle o `curl` para a porta 8000 da Hostinger devolve código **000**. Como o front depende de `rewrites()` para `/api`, o `next dev` do v2 **roda na Hostinger, porta 3008**. A Oracle fica como oficina do que não precisa do back: `next build` de verificação, componente isolado, spike. Seguem valendo: **build ARM não atravessa** (código sim, `node_modules` e `.next` não) e o **gate mede contra o back da Hostinger**, onde o produto vive — medir na máquina folgada e virar a chave na apertada fraudaria o próprio gate
