...
async def test_patch_esforco_obedece_override(monkeypatch):
    ...
    monkeypatch.setattr(agents, "_get_agent_or_404", AsyncMock(return_value={
        "motor_familia": "codex-proxy", "model_family": "anthropic",
    }))
    ...
    assert resposta.source == "agent_state.codex_reasoning_effort"
    db.update_agent_runtime_state.assert_awaited_once_with("teste", codex_reasoning_effort="high")


def test_leitura_velha_usa_pedido(monkeypatch):
    monkeypatch.setattr(agents.proxy_catalog, "listar_modelos", lambda: ("gpt-6-astra[1m]",))
    ctx = contexto("gpt-5.6-sol[1m]")
    ctx.stale = True
    painel = agents._build_painel_model(
        {"motor_familia": "codex-proxy", "state_model": "gpt-6-astra[1m]"}, ctx,
    )
    assert painel.value == "gpt-6-astra[1m]"
    assert painel.session_may_diverge
...
@pytest.mark.parametrize("familia,modelo,codigo", [
    ("codex-proxy", "gpt-6-astra[1m]", 200),
    ("opencode", "opus", 422), ("codex-proxy", "opus", 422),
])
...
def test_agente_sem_override_tambem_converge():
    ...
    motor = agents._painel_motor({"model_family": "codex-proxy"}, status_com("gpt-6-astra[1m]"))
